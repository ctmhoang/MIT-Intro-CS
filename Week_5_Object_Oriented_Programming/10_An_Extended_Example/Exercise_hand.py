...
class Hand(object):
    ...
    def setDummyHand(self, handString):
        ...
        '''
        assert len(handString) == self.HAND_SIZE, "Length of handString ({0}) must equal length of HAND_SIZE ({1})".format(len(handString), self.HAND_SIZE) #Replace {} to format of str
        self.hand = {}
...
#        raise NotImplementedError()
        wordList = dict(Counter(word))
        clone = self.hand.copy()
        if bool(0 not in [0 for i in clone if wordList.get(i,0) > clone.get(i,0)]) == True:
            self.hand = {key: self.hand[key] - wordList.get(key,0) for key in self.hand}
            return True
        else:
            return False
